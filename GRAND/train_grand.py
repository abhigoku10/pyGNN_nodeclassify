...
def main():
    parser = argparse.ArgumentParser(description="GRAND Node Classification")

    parser.add_argument('--config_path', action='store_true', \
    default='E:\\Freelance_projects\\GNN\\Tutsv2\\pyGNN_NC_XAI_V2\\GRAND\\config\\grand_pubmed.yaml', help='Provide the config path')
    ...
    dropnode_rate =configs['Model']['dropnode_rate']
    ip_droprate=configs['Model']['input_droprate']
    hid_droprate=configs['Model']['hidden_droprate']
    ...
    sample = configs['Hyper']['sample']
    orderr = configs['Hyper']['order']
    lam=configs['Hyper']['lam']
    temp = configs['Hyper']['tem']
